...
public class TableroController {
    ...
    @FXML
    private VBox playersStatusContainer;
    ...
    private StackPane crearNodoCasilla(Casilla c) {
        ...
        if (nombreCasilla.equals("NORMAL"))
            nombreCasilla = ""; // Dejar vacío si es normal
    ...
    private void moverJugadorAnimado(Jugador j, int pasosRestantes, Runnable onComplete) {
        ...
        if (cellAntigua != null)
            cellAntigua.getStyleClass().remove("casilla-active");
        if (cellNueva != null)
            cellNueva.getStyleClass().add("casilla-active");
        ...
        pause.setOnFinished(e -> {
            if (pasosRestantes <= 1 || posNueva >= Tablero.TAMANYO_TABLERO - 1) {
                if (cellNueva != null)
                    cellNueva.getStyleClass().remove("casilla-active");
            }
    ...
    private void actualizarUI() {
        ...
        playerStatusCards.forEach((j, card) -> {
            ...
            if (j == jActual)
                card.getStyleClass().add("player-active-card");
        });
    }
    ...
    private String getStyleClassForCasilla(String tipo) {
        switch (tipo) {
            case "Casilla OSO":
                return "casilla-oso";
            case "Casilla AGUJERO":
                return "casilla-agujero";
            case "Casilla TRINEO":
                return "casilla-trineo";
            case "Casilla INTERROGANTE":
                return "casilla-interrogante";
            case "Casilla ROMPEDIZAS":
                return "casilla-rompediza";
            default:
